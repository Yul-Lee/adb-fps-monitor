...
![启动预热](docs/screenshot_startup.png)

![实时监控](docs/screenshot.png)

## 功能

- **FPS 实时曲线** — SmartFPS 状态机自动探测最佳数据源（sf_buffer > timestats > sf_latency > gfxinfo）
...
- **CSV 数据录制** — 一键录制，导出 timestamp + FPS + 帧时间 + 温度 + 网络等全部数据
- **传感器选择面板** — 右侧 Dock，复选框控制曲线显隐
- **预热就绪系统** — 所有数据源就绪后统一开始监控，图表从 x=0 起始
- **全局统一时钟** — 所有 Worker 共用 `time.monotonic()` 时间基准
- **加载预采集** — 边加载边读数据，就绪后一次性注入
- **统一 60 秒时间轴** — 所有图表联动，支持左右拖拽查看历史
- **启动遮罩** — 数据预采集期间主窗口内遮罩，支持取消操作
...
4. **先启动目标游戏/应用**，再运行本工具（程序会自动检测前台应用并采集 FPS）
...
```
adb_fps_monitor.py     主程序入口（参数解析 + 设备检测 + 启动 GUI）
...
gui/
  __init__.py          包初始化
  main_window.py       主窗口（UI 布局、图表、预热系统、数据同步、录制）
  worker.py            Worker 线程（FPSWorker + GenericSensorWorker 泛型传感器 + DeviceInfoWorker）
  widgets.py           自定义组件（StatCard / CrosshairChart / TimeAxisWidget / LoadingOverlay）
...
```
启动 → ADB 设备检测 → 显示遮罩 → DeviceInfoWorker 后台获取设备信息
→ 更新遮罩设备信息 → Worker 后台预热
→ 各 Worker 首次有效数据 → ready 信号 → (6/6) 全部就绪
→ 统一清空图表 / 重置统计 / t=0 → 正式监控
```
...
| Exynos W1000 (erd5535) | Galaxy Watch7 | ✅ gfxinfo | ✅ thermalservice | ✅ 5核 | ✅ Mali devfreq | ✅ batterystats 回退 |
...
A: 确保在启动工具**之前**已打开游戏并进入实际游戏画面（非主菜单）。SmartFPS 会在无帧时自动切换数据源，进入暂停后每 3 秒自动重试恢复。

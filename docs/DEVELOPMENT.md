...
**技术栈：** Python 3.10+ / PyQt6 / pyqtgraph / ADB + Android App (Java)

**代码规模：** ~4400 行（核心模块）+ ~500 行（配套 App）
...
core/
  adb.py                    ADB 工具层（190 行）
  app_reader.py             配套 App HTTP 客户端（120 行）
...
gui/
  ...
  widgets.py                UI 组件（850 行）
  recorder.py               CSV 录制（240 行）
  help/                     帮助页面 HTML（8 个文件）
companion-app/              配套 Android App
  app/src/main/java/com/adbmonitor/companion/
    server/NanoHTTPD.java           嵌入式 HTTP 服务器
    server/MonitorHttpServer.java   路由层
    monitor/PowerMonitor.java       BatteryManager API
    monitor/MemoryMonitor.java      ActivityManager API
    monitor/NetworkMonitor.java     TrafficStats API
    MonitorService.java             前台 Service
    MainActivity.java               极简 UI
tests/
...
| `run_host(*args, timeout=5)` | 执行 ADB 主机端命令（forward、push 等） |
...
| `SettingsPanel` | 浮动传感器选择窗口（3 列网格复选框，传感器排序） |
| `HelpDialog` | 帮助对话框（Accordion 折叠，QTextBrowser 渲染） |
| `AccordionSection` | 可折叠内容区块（eventFilter 确保高度准确） |
...
### 8. `core/app_reader.py` — 配套 App 数据读取器

**AppDataReader**：通过 HTTP 读取配套 App 采集的数据。

| 方法 | 说明 |
|------|------|
| `start_app(package)` | `am start` 启动 App（传目标包名） |
| `is_running()` | `pidof` 检测 App 进程 |
| `probe()` | ADB forward + HTTP `/api/ping` |
| `read_power()` | 功耗：电流/电压/功率/电量 |
| `read_memory()` | 内存：系统级 total/avail |
| `read_network()` | 网络：上下行速率 |

**通信方式：** ADB 端口转发 `tcp:18765`，HTTP GET `/api/data[?kind=xxx]`

**数据合并策略（`_MergedMemReader`）：**
- 功耗：App 替代 ADB（BatteryManager 精度更高）
- 网络：App 替代 ADB（TrafficStats per-uid，不可用时回退系统级）
- 内存：App + ADB 合并（App 系统内存 + ADB GPU 显存/PSS）
...
```
companion-app/
  MonitorService (前台 Service + WakeLock)
    ├── MonitorHttpServer (com.sun.httpserver 替代品)
    │     └── NanoHTTPD (嵌入式，逐字节读取兼容 ADB 转发)
    ├── PowerMonitor (BatteryManager API)
    ├── MemoryMonitor (ActivityManager API)
    └── NetworkMonitor (TrafficStats API，per-app 不可用时回退系统级)
  MainActivity (透明主题，自动启 Service 后 finish)
```

**关键设计：**
- 前台 Service 防止 MIUI 等系统冻结/回收
- 透明 Activity + 延迟 `finish()` 避免目标应用切到后台
- NanoHTTPD 逐字节读取 HTTP 请求（`BufferedReader` 预读会导致 ADB 转发丢数据）
- NetworkMonitor per-app 返回 -1 时自动回退到 `TrafficStats.getTotalRxBytes()`
- `QUERY_ALL_PACKAGES` 权限解决 Android 11+ 包可见性限制
